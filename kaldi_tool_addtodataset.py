...
def main():
    ...
    for fp in dealfiles:
        ...
        wf.write('NEWG10_M%06dF NEWG10_M%06dF__%s\n' % (runint, runint, fpname.split('.')[0]))
    ...
    for fp in dealfiles:
        fpname = os.path.basename(fp)        
        wf.write('NEWG10_M%06dF__%s NEWG10_M%06dF\n' % (runint, fpname.split('.')[0], runint))
    ...
    for fp in dealfiles:
        ...
        wf.write('NEWG10_M%06dF__%s {relative path}/%s\n' % (runint, fpname.split('.')[0], fp))
